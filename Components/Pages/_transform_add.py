...
with open(r'G:\softwer\ERP.Hub\Components\Pages\AddEmployee.razor', 'r', encoding='utf-8') as f:
    orig = f.read()
...
# ============================================================
m = re.search(
    r'(\s*<MudTabs\s+@bind-ActivePanelIndex="(\w+)"[^>]*>)(.*?)(</MudTabs>\s*)',
    orig, re.DOTALL
)
if not m:
...
# ============================================================
# TRANSFORM: Strip MudXxx from panel content (one pass)
# ============================================================
def transform(txt):
    # 0. Protect C# lambda => from being treated as HTML tag close (> in => breaks [^>]*)
    ...
    # 6. MudSelect (with @bind-Value, possibly with Disabled)
    txt = re.sub(
    ...
    def _sel_str(m):
        var = m.group(1)
        rest = m.group(2)
        hm = re.search(r'\b(On\w+)\(', rest)
        handler = hm.group(1) if hm else 'OnGenderChanged'
    ...
    def _sel_int(m):
        var = m.group(1)
        rest = m.group(2)
        # Extract handler name from ValueChanged portion
        hm = re.search(r'\b(On\w+)\(\w+\)', rest)
    ...
    # 7. MudTextField (@bind-Value with/without Placeholder)
    def _tf_bind(m):
        bind = m.group(1)
        rest = m.group(2)
    ...
    # MudTextField (Value/ValueChanged with Placeholder)
    def _tf_val(m):
        var = m.group(1)
        rest = m.group(2)
        ...
        ml = re.search(r'MaxLength="([^"]*)"', rest)
        ms = f' maxlength="{ml.group(1)}"' if ml else ''
        return f'<input type="text" value="@{var}" @onchange="@( (ChangeEventArgs e) => {handler}(e.Value?.ToString() ?? "") )" class="form-control"{ps}{ms} />'
    txt = re.sub(r'<MudTextField\s+Value="(\w+)"([^>]*)>', _tf_val, txt, flags=re.DOTALL)
    txt = re.sub(r'</MudTextField>', '', txt)

    # 8. MudNumericField (@bind-Value with/without Placeholder)
    def _nf_bind(m):
        bind = m.group(1)
        rest = m.group(2)
        pl = re.search(r'Placeholder="([^"]*)"', rest)
    ...
    # MudNumericField (Value with disabled)
    def _nf_val(m):
        var = m.group(1)
        rest = m.group(2)
        ...
        # Try to find handler from ValueChanged
        hm = re.search(r'(\w+)\(', rest[rest.find('ValueChanged'):]) if 'ValueChanged' in rest else None
        handler = hm.group(1) if hm else None
        if handler:
            return f'<input type="number" value="@{var}" @onchange="@( (ChangeEventArgs e) => {{ decimal.TryParse(e.Value?.ToString() ?? "0", out var v); {handler}(v); }} )" class="form-control"{ps}{min_attr}{disabled} />'
        return f'<input type="number" value="@{var}" class="form-control"{ps}{min_attr}{disabled} />'
    txt = re.sub(r'<MudNumericField\s+Value="([^"]+)"([^>]*)>', _nf_val, txt, flags=re.DOTALL)
    txt = re.sub(r'</MudNumericField>', '', txt)

    # 9. MudDatePicker
    txt = re.sub(r'<MudDatePicker\s+Date="_dob"[^>]*>', r'<input type="date" @bind-Value="_dob" class="form-control" />', txt, flags=re.DOTALL)
    txt = re.sub(r'<MudDatePicker\s+Date="@_joiningDate"[^>]*>', r'<input type="date" @bind-Value="_joiningDate" class="form-control" />', txt, flags=re.DOTALL)
    ...
    def _cb(m):
        ...
        label = lm.group(1) if lm else ''
        hm = re.search(r'(\w+)\(', rest[rest.find('ValueChanged'):])
        handler = hm.group(1) if hm else 'OnSameAsPresentChanged'
    ...
    # 14. MudProgressCircular
    txt = re.sub(r'<MudProgressCircular[^>]*/>', '<div class="flex justify-center items-center"><span class="spinner"></span></div>', txt, flags=re.DOTALL)
...
for i, p in enumerate(panels):
    ...
    # Re-indent: strip existing indent, add INDENT
...
with open(r'G:\softwer\ERP.Hub\Components\Pages\AddEmployee.razor', 'w', encoding='utf-8') as f:
    f.write(result)
