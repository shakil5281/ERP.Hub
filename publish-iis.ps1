...
<#
.SYNOPSIS
    Publishes the .NET Blazor application and hosts it on local IIS.
.DESCRIPTION
    This script automates the full deployment cycle for ERPHub:
    1. Validates prerequisites (.NET SDK, ASP.NET Core Hosting Bundle).
    2. Runs dotnet publish in Release mode to the target directory.
    3. Enables IIS features if not already configured.
    4. Configures IIS Application Pool ("No Managed Code") and Web Site.
    5. Configures ACL permission for IIS_IUSRS access.
#>
...
if (-not (Get-Command "dotnet" -ErrorAction SilentlyContinue)) {
    Write-Error "dotnet CLI was not found. Please install the .NET SDK."
}
...
if (-not $hasHostingBundle) {
    ...
    Write-Host "  Please install the ASP.NET Core Hosting Bundle: https://dotnet.microsoft.com/en-us/download/dotnet" -ForegroundColor Yellow
...
# 4. dotnet publish
Write-Host "`n[2/5] Publishing application..." -ForegroundColor Cyan
...
# 5. Enable IIS Windows Features
Write-Host "`n[3/5] Checking Windows Features (IIS)..." -ForegroundColor Cyan
...
# 6. Configure IIS Web Site and AppPool
Write-Host "`n[4/5] Configuring IIS Web Site & AppPool..." -ForegroundColor Cyan
...
# 7. Set File System Permissions for IIS_IUSRS
Write-Host "`n[5/5] Applying folder security permissions..." -ForegroundColor Cyan
...
if (-not $hasHostingBundle) {
    Write-Host "`n [ALERT] Remember to install the ASP.NET Core Hosting Bundle!" -ForegroundColor Yellow
